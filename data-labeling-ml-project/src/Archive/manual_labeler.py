...
def label_jumps_with_ui(data, value_key="z"):
    # Use index as a synthetic timestamp if 'timestamp' is missing
    if "timestamp" in data[0]:
        timestamps = [entry["timestamp"] for entry in data]
    else:
        timestamps = list(range(len(data)))

    # Find all numeric keys except 'timestamp'
    sample_entry = data[0]
    keys = [k for k in sample_entry.keys() if k != "timestamp" and isinstance(sample_entry[k], (int, float))]

    fig, ax = plt.subplots()
    for key in keys:
        values = [entry.get(key, 0) for entry in data]
        ax.plot(timestamps, values, label=key, alpha=0.7, linewidth=2 if key == value_key else 1)
    ax.set_title(f"Click to mark start and end of jumps (pairs) - Labeling: {value_key}")
    ax.set_xlabel("Index" if "timestamp" not in data[0] else "Timestamp")
    ax.set_ylabel("Value")
    plt.legend()
